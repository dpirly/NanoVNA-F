...
/*
 * CELL_X0[27:31] cell position 0-31
 * CELL_Y0[22:26] cell position 0-31
 * CELL_N[10:21]  original order 0-4095
 * CELL_X[5:9]    position in the cell 0-31
 * CELL_Y[0:4]    position in the cell 0-31
 */
...
#define CELL_X0(i)         (int)(((i)>>22)&0x03e0)  // 10bit中的高5bit
#define CELL_Y0(i)         (int)(((i)>>17)&0x03e0)  // 10bit中的高5bit
...
uint32_t trace_into_index(int x, int t, int i, float coeff[2])
{
  ...
  switch (trace[t].type) {
  case TRC_LOGMAG:
    v = refpos - logmag(coeff) * scale;  // t=1, 1-(-3.1) logmag(0)=0
    break;
  case TRC_PHASE:
...
gamma2imp(char *buf, int len, const float coeff[2], uint32_t frequency)
{
  ...
  float zr = ((1+coeff[0])*(1-coeff[0]) - coeff[1]*coeff[1]) * d;  // real
  float zi = 2*coeff[1] * d;  // imag
  int n;
...
trace_get_info(int t, char *buf, int len)
{
  ...
  case TRC_LOGMAG:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %ddB/", "\x7F\x80\x81\x82\x83\x84", (int)(trace[t].scale*10));
    else
      chsnprintf(buf, len, "%s %ddB/", type, (int)(trace[t].scale*10));
    break;
  case TRC_PHASE:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %d" S_DEGREE "/", "\x85\x86\x81\x82\x83\x84", (int)(trace[t].scale*90));
    else
      chsnprintf(buf, len, "%s %d" S_DEGREE "/", type, (int)(trace[t].scale*90));
    break;
  case TRC_DELAY:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %.1f/", "\x87\x88\x89\x8A\x8B\x8C", trace[t].scale);
    else
  ...
  //case TRC_ADMIT:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %.1fFS", "\x8D\x8E\x8F\x90\x91\x92", trace[t].scale);
    else
      chsnprintf(buf, len, "%s %.1fFS", type, trace[t].scale);
    break;
  case TRC_POLAR:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %.1fFS", "\x93\x94\x95\x96\x97\x98", trace[t].scale);
    else
      chsnprintf(buf, len, "%s %.1fFS", type, trace[t].scale);
    break;
  case TRC_LINEAR:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %.1f/", "\x7F\x80\x81\x82\x83\x84", trace[t].scale);
    else
      chsnprintf(buf, len, "%s %.1f/", type, trace[t].scale);
    break;
  case TRC_SWR:
    if (config.lang == LANG_CN)
      chsnprintf(buf, len, "%s %.1f/", "\x99\x9A\x9B\x9C\x9D\x9E", trace[t].scale);
    else
      chsnprintf(buf, len, "%s %.1f/", type, trace[t].scale);
    break;
  default:
...
{
  if (y >= 0 && y < 8 && x >= 0 && x < 16)  // 对应 8*32=256 16*32=512
...
void plot_into_index(float measured[2][SWEEP_POINTS][2])
{
  ...
  for (i = 0; i < sweep_points; i++) {
    int x = i * (WIDTH-1) / (sweep_points-1);  // WIDTH 为曲线区域宽度
    for (t = 0; t < TRACES_MAX; t++) {
      if (!trace[t].enabled)
        continue;
...
void draw_cell(int m, int n)
{
  ...
  for (t = 0; t < TRACES_MAX; t++) { // 共 4 条曲线
    if (!trace[t].enabled)
      continue;
...
draw_all_cells(void)
{
  ...
  for (m = 0; m < (area_width+CELLWIDTH-1) / CELLWIDTH; m++)  // 0-9 -> 0-10
    for (n = 0; n < (area_height+CELLHEIGHT-1) / CELLHEIGHT; n++) {  // 0-8
      if (is_mapmarked(m, n)) // 只绘制标记的CELL
        draw_cell(m, n);
      //ui_process();
...
void
cell_drawstring_06x13(int w, int h, char *str, int x, int y, uint16_t fg)
{
  while (*str) {
    cell_drawchar_06x13(w, h, *str, x, y, fg, FALSE);
    x += 6;
    str++;
  }
}
...
void
cell_drawstring_invert_06x13(int w, int h, char *str, int x, int y, uint16_t fg, int invert)
{
  while (*str) {
    cell_drawchar_06x13(w, h, *str, x, y, fg, invert);
    x += 6;
    str++;
  }
}
...
void cell_draw_marker_info(int m, int n, int w, int h)
{
    ...
    for (t = 0; t < TRACES_MAX; t++) {
        ...
        ypos -= n * CELLHEIGHT;
        if (trace[t].channel == 0) {
            chsnprintf(buf, sizeof buf, "S11");
        } else {
            chsnprintf(buf, sizeof buf, "S21");
        }
        if (n == 0)
        {
            #if USE_ILI_LCD
            ...
            cell_drawstring_5x7(w, h, buf, xpos, ypos, config.trace_color[t]);
            #else
            cell_drawstring_invert_06x13(w, h, buf, xpos, ypos, config.trace_color[t], t == uistat.current_trace);
            xpos += 22;
            trace_get_info(t, buf, sizeof buf);
    ...
    ypos -= n * CELLHEIGHT;
    chsnprintf(buf, sizeof buf, "%d:", active_marker + 1);
    cell_drawstring_06x13(w, h, buf, xpos, ypos, 0xffff);
    xpos += 19;
    frequency_string(buf, sizeof buf, frequencies[idx]);
    cell_drawstring_06x13(w, h, buf, xpos, ypos, 0xffff);
...
#define YSTEP 7
  ...
  int y = 80*2;
#define YSTEP 24
  nt35510_fill(0, y, 12*2, 24*6, 0x0000);
  if (cal_status & CALSTAT_APPLY) {
    char c[3] = "C0";
    if (cal_status & CALSTAT_INTERPOLATED)
      c[0] = 'c';
...
draw_battery_status()
{
    ...
    uint16_t col = vbati == 0 ? BRG556(0, 255, 0) : BRG556(0, 0, 240);
    memset(lcd_buffer, 0, w * h * 2);
